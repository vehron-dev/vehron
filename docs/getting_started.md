...
If you are using VEHRON as an installed package and want the quickest first run,
...
```bash
vehron run-example --vehicle bev_car_sedan --testcase wltp_class3b_standard
```
...
This custom CSV workflow is still repository-oriented in v1 because it depends
on user-authored local files. The packaged `run-example` command is for the
built-in baseline studies only.
